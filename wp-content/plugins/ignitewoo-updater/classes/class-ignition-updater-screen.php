...
class Ignition_Updater_Screen {
	...
	public static function get_header ( $token = 'ignition-updater', $screen_icon = 'tools' ) {
		...
		$html .= '</h2>' . "\n";
		echo $html;
		do_action( 'ignition_updater_screen_header_before_content', $token, $screen_icon );
	} // End get_header()
	...
	public static function get_footer ( $token = 'ignition-updater', $screen_icon = 'tools' ) {
		...
		echo $html;
		do_action( 'ignition_updater_screen_after', $token, $screen_icon );
	} // End get_footer()
	 ...
	 */
	public static function get_navigation_tabs ( $menu_key = 'ignition' ) {
		$html = '';

		$screens = Ignition_Updater_Screen::get_available_screens();
	...
	public static function get_available_screens () {
		return array(
			'subscriptions' => __( 'Licenses', 'ignition-updater' ),
			'help' => __( 'Help', 'ignition-updater' )
			);
